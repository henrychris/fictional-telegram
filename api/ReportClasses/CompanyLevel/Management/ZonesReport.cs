...
namespace api.ReportClasses.CompanyLevel.Management
{
    ...
    ///</summary>
    public class ZonesReport : PortraitTemplate
    {
        // https://api.epump.com.ng/Zones/Company/b2e245d3-dd83-40fd-8bd5-53bd32810744
        public byte[] getPdf(List<Company> companyList, List<Zones> zoneList)
        {
            writer = new PdfWriter(pdfstream);
            // writer = new PdfWriter("./Files/PDFS/Test/zonesReport-test.pdf");
            ...
            Table dataTable = new Table(4, true);
            dataTable.SetMaxWidth(717);

            string[] itemHeaders = { "S/N", "Zone Name", "No. Of Outlets Assigned", "Date Created" };
            ...
            // Orders by most outlets assigned
            zoneList = zoneList.OrderBy(x => x.branches).Reverse().ToList();
            ...
            // add items to table.
            {
                ...
                new Cell().SetBorder(Border.NO_BORDER).Add(new Paragraph($"{sn.ToString()}")));
                    ...
                    dataTable.AddCell(new Cell().SetBorder(Border.NO_BORDER).Add(new Paragraph($"{item.branches}"))).SetFontSize(10).SetTextAlignment(TextAlignment.CENTER);
                ...
                new Cell().SetBorder(Border.NO_BORDER).SetBackgroundColor(tableBackgroundColor).Add(new Paragraph($"{sn.ToString()}")));
                    ...
                    dataTable.AddCell(new Cell().SetBorder(Border.NO_BORDER).SetBackgroundColor(tableBackgroundColor).Add(new Paragraph($"{item.branches}"))).SetFontSize(10).SetTextAlignment(TextAlignment.CENTER);
